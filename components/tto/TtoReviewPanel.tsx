...
export const TtoReviewPanel: React.FC<{ disclosureId: string; onCompleted?: () => void }> = ({ disclosureId, onCompleted }) => {
  ...
  return <div className="disclosure-page space-y-6 rounded-2xl bg-slate-50 p-5 sm:p-6">
     <header><p className="disclosure-eyebrow text-ug-teal">{t('dashboard.ttoIpOffice')} review · {disclosureId.slice(0, 8)}</p><h4 className="disclosure-title mt-1">{caseData?.project?.title || t('common.loading')}</h4><p className="disclosure-body mt-2">{caseData?.project?.description || t('common.details')}</p></header>
    <section className="disclosure-section tto-answer-grid"><h5 className="disclosure-eyebrow">IP question answers</h5><div className="mt-4 grid gap-x-8 gap-y-1 md:grid-cols-2">{Object.entries(caseData?.disclosure.answers || {}).map(([key, value]) => <div key={key} className="border-b border-slate-200 py-4"><span className="answer-label">{key.replaceAll('_', ' ')}</span><p className="answer-value mt-1 whitespace-pre-line">{formatIpAnswer(value)}</p></div>)}</div></section>
     <section className="disclosure-section"><h5 className="disclosure-eyebrow text-ug-teal">AI advisory findings</h5><p className="disclosure-note mt-1">Advisory only. AI output does not grant legal clearance.</p><div className="mt-3 divide-y divide-slate-200">{findings.filter((finding) => finding.source_type === 'ai').map((finding) => <article key={finding.id} className="py-4"><p className="text-sm font-bold text-ug-navy">{finding.title} <span className="ml-1 text-[10px] font-bold uppercase text-slate-400">{finding.severity}</span></p><p className="mt-2 whitespace-pre-line text-[13px] leading-6 text-slate-600">{linkedBody(finding.body)}</p></article>)}{!findings.some((finding) => finding.source_type === 'ai') && <p className="disclosure-empty mt-3">{t('dashboard.aiScreeningPending')}</p>}</div></section>
