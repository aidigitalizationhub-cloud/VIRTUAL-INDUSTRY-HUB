...
export const DashboardHeader: React.FC<DashboardHeaderProps> = ({
  ...
  <DashboardHeaderContent displayName={displayName} administrative={administrative} activeTab={activeTab} unreadCount={unreadCount} onMessages={onMessages} showMessages={showMessages} onLogout={onLogout} onNavigate={onNavigate} />
);

const DashboardHeaderContent: React.FC<DashboardHeaderProps> = ({ displayName, administrative, activeTab, unreadCount, onMessages, showMessages = true, onLogout, onNavigate }) => {
  const { t } = useTranslation();
  return <header className="bg-ug-navy text-white flex items-center justify-between px-4 sm:px-8 py-3 sm:py-4 shrink-0 shadow-xl z-50 border-b border-white/10">
    <button className="flex sm:hidden items-center gap-2 group" onClick={() => onNavigate('/')} title="Return to Home">
      <span className="w-8 h-8 rounded-xl bg-gradient-to-br from-ug-teal via-teal-600 to-teal-800 flex items-center justify-center text-white shadow-md shadow-teal-900/40 ring-1 ring-white/20 group-hover:scale-105 transition-transform"><Home size={16} /></span>
       <span className="font-bold text-xs tracking-tight group-hover:text-ug-teal transition-colors">{t('nav.brand')}</span>
    </button>
    <nav className="hidden sm:flex items-center gap-6 lg:gap-8 ml-0 lg:ml-8">
       {!administrative && [['nav.home', '/'], ['nav.projects', '/projects'], ['nav.products', '/products'], ['nav.news', '/news']].map(([key, path]) => (
        <button key={key} onClick={() => onNavigate(path)} className="text-xs font-bold hover:text-ug-teal transition-colors text-white/80">{t(key)}</button>
      ))}
    </nav>
    {displayName && (
      <div className="hidden md:flex items-center gap-2.5 px-4 py-1.5 bg-white/5 rounded-full border border-white/10 text-xs shadow-inner">
         <span className="font-light text-white/50">{t('common.welcome')},</span>
