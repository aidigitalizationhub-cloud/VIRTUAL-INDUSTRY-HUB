...
import {
  ...
  createIpDisclosureRequestSchema,
  submitIpDisclosureRequestSchema,
  listIpDisclosuresQuerySchema,
  patchIpDisclosureRequestSchema,
  adminReturnIpDisclosureRequestSchema,
  createIpFindingRequestSchema,
  shareIpFindingRequestSchema,
  createIpLinkRequestSchema,
  publicationDecisionRequestSchema,
  registerIpFileRequestSchema,
...
import { applyTransition, isUuid, missingTables } from './server/ip/ipService';
import { canViewDisclosure, canAdminReview, canTtoReview, canDecidePublication, isTtoRole, isAdminRole } from './server/ip/ipAuthz';
import { writeIpEvent } from './server/ip/ipAudit';
import { buildAdvisoryFindings } from './server/ip/ipAi';
import { collectIpEvidence, formatIpEvidenceForPrompt, formatIpReviewerFinding } from './server/ip/ipEvidence';
...
const generateWithAssistantReviewer = async (prompt: string): Promise<{ provider: string; model: string; text: string } | null> => {
...
const getAssistantReviewerClient = (): GoogleGenAI | null => {
...
app.get('/api/industry-challenges', async (req, res) => {
  try {
    ...
    const formatted = (challenges || []).map((ch: any) => ({
      ...ch,
      partner_name: partnerMap.get(ch.partner_id)?.name || 'Industry Partner',
      partner_company: partnerMap.get(ch.partner_id)?.company || 'Partner Org'
    }));
