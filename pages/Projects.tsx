  ...
  return (
    <div className="min-h-screen bg-gray-50 py-12">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="mb-10">
          <h1 className="text-4xl font-bold text-ug-navy tracking-tight"><Tr text="Research Pipeline" /></h1>
        ...
        {/* Filters & Search */}
        <div className="bg-white p-4 md:p-6 rounded-2xl shadow-sm border border-gray-100 mb-8 relative">
          <div className="relative w-full">
            ...
            <button
              ...
              className={`absolute right-2 top-1/2 transform -translate-y-1/2 py-2 px-3 sm:px-4 rounded-xl text-xs font-bold uppercase tracking-wider flex items-center gap-1.5 transition-all duration-300 ${
            ...
            >
              ...
              {activeFiltersCount > 0 && (
                <span className="w-4 h-4 bg-white text-ug-navy rounded-full flex items-center justify-center text-[11px] font-semibold leading-none shrink-0">
                  {activeFiltersCount}
                </span>
              )}
            </button>
          ...
          {activeFiltersCount > 0 && (
            <div className="flex flex-wrap items-center gap-2 mt-4 pt-4 border-t border-gray-100">
              <span className="text-[11px] font-semibold tracking-wide text-gray-400 mr-1"><Tr text="Active Filters:" /></span>
          ...
          {showFilters && (
            <div className="absolute right-4 md:right-6 top-full mt-3 w-[calc(100%-2rem)] sm:w-[460px] bg-white dark:bg-gray-900 rounded-2xl shadow-xl border border-gray-200 dark:border-gray-800 p-5 z-50 transition-all duration-200">
              <div className="flex items-center justify-between pb-3 mb-4 border-b border-gray-100 dark:border-gray-800">
                <span className="text-xs font-bold uppercase tracking-wide text-ug-navy dark:text-white"><Tr text="Filter Research Pipeline" /></span>
                <div className="flex items-center gap-2">
                  {activeFiltersCount > 0 && (
                    <button 
                      ...
                      className="text-[11px] font-bold text-gray-400 hover:text-red-500 tracking-wider flex items-center gap-1 transition-colors cursor-pointer"
                    >
                      <RotateCcw size={10} />
                      <Tr text="Reset" />
                    </button>
              ...
              <div className="space-y-4">
                ...
                <div className="space-y-1.5">
                  <label className="text-[11px] font-bold text-gray-400 tracking-wider block flex items-center gap-1">
                ...
                {/* Research track */}
                <div className="space-y-1.5">
                  <label className="text-[11px] font-bold text-gray-400 tracking-wider block"><Tr text="Research Track" /></label>
                  <div className="relative">
                    <select 
                      className="appearance-none w-full bg-gray-50 dark:bg-gray-800 border border-gray-200 dark:border-gray-700 text-gray-800 dark:text-gray-200 py-2.5 px-4 pr-10 rounded-xl font-bold text-xs focus:outline-none focus:ring-2 focus:ring-ug-teal transition-all cursor-pointer"
                      value={selectedArea}
                ...
                {/* Development stage */}
                <div className="space-y-1.5">
                  <label className="text-[11px] font-bold text-gray-400 tracking-wider block"><Tr text="Development Stage" /></label>
                  <div className="relative">
                    <select 
                      className="appearance-none w-full bg-gray-50 dark:bg-gray-800 border border-gray-200 dark:border-gray-700 text-gray-800 dark:text-gray-200 py-2.5 px-4 pr-10 rounded-xl font-bold text-xs focus:outline-none focus:ring-2 focus:ring-ug-teal transition-all cursor-pointer"
                ...
                {/* Sort order */}
                <div className="space-y-1.5">
                  <label className="text-[11px] font-bold text-gray-400 tracking-wider block"><Tr text="Sort Projects" /></label>
                  <div className="relative">
                    <select 
                      className="appearance-none w-full bg-gray-50 dark:bg-gray-800 border border-gray-200 dark:border-gray-700 text-gray-800 dark:text-gray-200 py-2.5 px-4 pr-10 rounded-xl font-bold text-xs focus:outline-none focus:ring-2 focus:ring-ug-teal transition-all cursor-pointer"
                      value={sortBy}
              ...
              <div className="mt-5 pt-3 border-t border-gray-100 dark:border-gray-800">
                <button 
                  onClick={() => setShowFilters(false)}
                  className="w-full bg-ug-navy hover:bg-ug-teal text-white py-2.5 rounded-xl text-xs font-bold uppercase tracking-wide transition-colors shadow-sm cursor-pointer"
                >
        ...
        {loading ? (
          <div className="flex flex-col items-center justify-center py-40">
             ...
             <p className="text-gray-400 text-xs font-bold uppercase tracking-wide"><Tr text="Loading scientific database..." /></p>
          </div>
        ) : (
          <>
            {/* Results Grid */}
                ...
                <div key={project.id} className="bg-white rounded-[2.5rem] overflow-hidden shadow-sm hover:shadow-xl transition-all duration-500 flex flex-col h-full border border-gray-100 group animate-fade-in-up">
                  <div className="h-64 relative overflow-hidden">
                     ...
                     <div className="absolute bottom-4 left-6 text-white">
                        <div className="text-xs opacity-90 font-bold uppercase tracking-wide"><Tr text={project.department} /></div>
                     </div>
                  </div>
                  <div className="p-8 flex-1 flex flex-col">
                    <h3 className="text-xl font-bold text-gray-900 mb-3 leading-tight group-hover:text-ug-teal transition-colors">
                    ...
                    {project.owner_name && (
                      <div className="mb-4 flex items-center gap-2">
                        {project.owner_avatar ? (
                          ...
                          <div className="w-6 h-6 rounded-full bg-ug-teal/10 text-ug-teal flex items-center justify-center font-bold text-[11px]">
                        ...
                        >
                          ...
                          <span className="text-[11px] text-ug-teal opacity-0 group-hover/author:opacity-100 transition-opacity font-semibold">
                            (Filter)
                          </span>
                        </button>
                      </div>
                    ...
                    <div className="mt-4 space-y-4 pt-6 border-t border-gray-100">
                       <div className="flex justify-between text-xs items-center">
                          <span className="text-gray-400 font-bold uppercase tracking-wide"><Tr text="Commercialization Stage" /></span>
                          <span className={`px-3 py-1 rounded-full border text-[11px] font-semibold tracking-wide ${getStatusColor(project.status)} shadow-sm`}>
                            <Tr text={project.status} />
                          </span>
                       </div>
                       <div className="flex justify-between text-xs items-center">
                          <span className="text-gray-400 font-bold uppercase tracking-wide"><Tr text="Budget Range" /></span>
                          <span className="font-bold text-ug-navy">{project.budget}</span>
                       </div>
                    </div>

                    <button 
                      onClick={() => navigate(`/projects/${project.id}`)}
                      className="mt-8 w-full py-4 bg-ug-navy text-white font-bold text-xs uppercase tracking-wide rounded-2xl hover:bg-ug-teal hover:scale-[1.02] active:scale-[0.98] transition-all shadow-xl shadow-ug-navy/10 flex items-center justify-center gap-2 cursor-pointer"
                    >
            ...
            {totalPages > 1 && (
              <div className="flex justify-center items-center gap-4 mt-16">
                <button 
                  ...
                  className="px-6 py-3.5 bg-white hover:bg-gray-100 rounded-xl border border-gray-200 text-gray-700 hover:text-ug-teal font-bold text-xs uppercase tracking-wide disabled:opacity-40 transition shadow-sm cursor-pointer"
                >
                  <Tr text="Previous" />
                </button>
