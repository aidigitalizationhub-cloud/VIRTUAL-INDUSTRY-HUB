...
- Verification note: the local 10 September 2026 test run passed 18 files / 100 tests; lint and production build also pass locally on the modular server layout (`server.ts` thin bootstrap). Live database and deployment verification remain required.
- News curation hardening (10 September 2026): deleted scout fallback insertion and saved-search localStorage alerts; live HTTP source retrieval with timeout/size caps gates every `Published` save; commercialization items enter as `Draft`; strict `Draft/Published` status via `docs/database/news_status_strict.sql`; degraded document extraction sets `needs_review` and blocks first publish; news card Source indicator links directly to the verified URL.
- Server modularization (10 September 2026): monolithic `server.ts` (~3,652 lines) split into `server/app.ts`, `server/config/env.ts`, `server/db/supabase.ts`, `server/middleware/{auth,rateLimit,security,validate}.ts`, `server/services/{aiGateway,sourceVerification}.ts`, and `server/routes/{health,profiles,news,ip,storage,projects,eois,ai,scout,challenges}.ts` with `server/app.test.ts` + `server/services/sourceVerification.test.ts`; API contracts preserved.
