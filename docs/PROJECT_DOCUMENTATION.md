...
3. **Hybrid AI matching** where deterministic, auditable local scoring is authoritative and an LLM enriches ranking with qualitative reasoning (Gemini primary, Groq fallback).
...
- `POST /api/ai-scout/sync` (Gemini-primary, Groq fallback) pulls vetted global biomedical source lists (`UG_SOURCES` + `GLOBAL_ACCREDITED`), upserts into `news`, and records the decision. `force` flag requires admin.
...
| Auth | Supabase Auth legacy support + Better Auth migration target |
| Access control | RLS policies, service-role server mediation for Better Auth routes, `SECURITY DEFINER` matching functions |

---

## 12. Data Model

Primary schema is provisioned by `docs/database/supabase_setup.sql`; `docs/database/supabase_rls_verify.sql` asserts policy state. Better Auth migration and table scripts live in `docs/database/`.
...
All protected routes except `/api/health` are validated by `authenticateUser`, which accepts Better Auth session cookies first and Supabase Bearer JWTs as the legacy fallback. Admin routes additionally require the `Admin` role.

| Method | Path | Auth | Purpose |
|---|---|---|---|
| GET | `/api/health` | — | Liveness (`{status:'ok', time}`) |
| GET | `/api/profile/me` | user | Server-backed current profile lookup for Supabase or Better Auth sessions |
| POST | `/api/profile/update` | user | Server-backed profile save for onboarding and Better Auth sessions |
| POST | `/api/matches` | user | Server-backed vector/fallback match retrieval for Better Auth sessions |
| POST | `/api/translate` | user | Text translation (Gemini) |
| POST | `/api/gemini/chat` | user | AI assistant chat (Groq fallback, Gemini fallback path retained) |
| POST | `/api/gemini/embed` | user, throttled | Embeddings via `gemini-embedding-2-preview` (768-d) |
| POST | `/api/ai-profile` | user | Extract/refine AI profile (Gemini-primary, Groq fallback) |
| POST | `/api/ai-scout/sync` | user; `force` = admin | Scout feed sync + news upsert (Gemini-primary, Groq fallback) |
...
- **Primary:** Gemini `gemini-3.6-flash`, `gemini-3.5-flash`, `gemini-flash-latest`, `gemini-3.1-flash-lite`.
- **Fallback:** Groq — model `openai/gpt-oss-120b` by default (override via `GROQ_MODEL`). Server-only key.
- Helper `generateWithProviders` (server.ts) tries Gemini then Groq and returns `{provider, model, text}`; `generateWithFallback` handles Gemini-only paths (translate, document extraction).
...
3. **Stage 3 — LLM enrichment:** Gemini-primary `generateWithProviders` produces JSON reasoning + alignment labels (validated against `matchRankingsSchema`); Groq is fallback only. The LLM output is merged into deterministic scores but cannot override them.
...
- `SUPABASE_SERVICE_ROLE_KEY`, `GEMINI_API_KEY`, `GROQ_API_KEY`, `BETTER_AUTH_SECRET`, and database URLs are **server-only**; the browser uses only intended `VITE_` variables and the Supabase anon key.
- The server validates the Supabase service key shape and returns explicit configuration errors when the key is missing or appears to be a non-Supabase provider key.
...
# AI
GEMINI_API_KEY=
GROQ_API_KEY=          # server-only fallback provider
...
- **AI cost/governance:** provenance ledger is the source of truth for provider usage; Gemini is primary and Groq remains a configurable fallback through `GROQ_MODEL`.
...
| Capability | Status |
|---|---|
| Gemini-primary / Groq-fallback AI | **Realized** |
