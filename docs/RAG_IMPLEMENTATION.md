...
2. **Stage 2 (Generative Synthesis & Re-ranking)**: Powered by **Gemini-primary generation with Groq fallback** (`generateWithProviders`). Deterministic local scoring is authoritative; the LLM supplies only qualitative reasoning and alignment labels.
...
<svg viewBox="0 0 900 480" width="100%" height="auto" xmlns="http://www.w3.org/2000/svg" style="background: #0f172a; border-radius: 16px; padding: 20px; font-family: system-ui, sans-serif;">
  ...
  <g filter="url(#shadow)">
    ...
    <text x="800" y="172" text-anchor="middle" fill="#f3e8ff" font-size="11">Gemini (Groq fb)</text>
...
flowchart TD
    ...
    subgraph Stage 2: Generative Re-ranking
        B3 --> C1[Gemini-primary LLM Ranker / Groq fallback]
...
### 3.2 Dimension Normalization & Empty-Vector Safety
Vector dimensions are strictly enforced to 768 dimensions. Empty or missing embeddings are never sent to pgvector; the app skips vector RPC calls and falls back to directory/project discovery instead:
...
The LLM (Gemini-primary, Groq fallback) contributes only `reasoning` and `alignment_label`; scores always come from the deterministic engine. Every run is recorded in `ai_decisions` (`decision_type: match_ranking`).
...
// 2. LLM supplies reasoning/label only (Gemini-primary, Groq fallback).
